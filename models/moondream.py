...
class MoondreamCaptioner:
    def __init__(self, torch_device = torch.device("cuda" if torch.cuda.is_available() else "cpu"), dtype = torch.float32):
        ...
        self._moondream_tokenizer = AutoTokenizer.from_pretrained("vikhyatk/moondream2", revision="2024-08-26")
        print("Tokenizer loaded")
        print("Loading model...")
        print(f"Dtype: {dtype}")
        self._moondream_model = AutoModelForCausalLM.from_pretrained(
            "vikhyatk/moondream2", trust_remote_code=True, revision="2024-08-26", torch_dtype = dtype, attn_implementation="flash_attention_2"
        ).to(self._device)
        print("Model loaded")
